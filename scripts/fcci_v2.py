...
def _FC_poisson(n0, b, t, conf=0.95, useCorrection=False, tol=5E-4,
    ...
    """
    Feldman Cousins confidence level calculation for the Poisson case with known mean background.
    (Memory‑efficient batched coarse scan with adaptive mumax expansion)
    """
    import numpy as np
    from scipy.stats import norm
    import warnings
    ...
    # ----- Initial mu range (conservative) -----
    # Use a generous base factor; will be increased if needed
    if n0 + bt <= 3:
        if mumax is None:
            mumax = 11.0
        if mumin is None:
        ...
        # Start with factor = 5 (original) – we will expand if necessary
        if mumax is None:
            mumax = n0 + bt + 5.0 * np.sqrt(n0 + bt)
        if mumin is None:
            mumin = 0.0

    # Determine the n range (same as original)
    if n0 <= 100:
        minn = 0
        maxn = int((2 * n0 + 16 + bt) * (sigma / 1.96 if conf > 0.95 else 1))
    else:
        minn = 0
        maxn = int(1.75 * n0 + bt)
    nn = np.arange(minn, maxn, dtype=int)

    # ----- Adaptive coarse scan: expand mumax until rough upper limit is inside -----
    max_expansions = 5           # safety limit
    expansion_factor = 2.0       # double each time
    roughstep = 200 * tol

    for attempt in range(max_expansions + 1):
        steps = max(10, int((mumax - mumin) / roughstep))
        murange = np.linspace(mumin, mumax, steps)

        # Reduce murange for large n0 (Gaussian approximation) – optional
        if n0 >= 50:
            CL_low_rough = n0 - np.sqrt(n0) * sigma + 1 + n0 * roughstep
            CL_high_rough = n0 + np.sqrt(n0) * sigma + 1 - n0 * roughstep
            murange = murange[(murange < CL_low_rough) | (murange > CL_high_rough)]

        # Batched coarse scan to find which mu values include n0
        batch_size = 500
        included = np.zeros(len(murange), dtype=bool)

        for start in range(0, len(murange), batch_size):
            end = min(start + batch_size, len(murange))
            mu_batch = murange[start:end]
            NN, MU = np.meshgrid(nn, mu_batch)
            ...
            ind = np.argsort(r, axis=1)[:, ::-1]
            cutoff_ind = np.sum(
                np.cumsum(pdf[np.arange(len(mu_batch))[:, None], ind], axis=1) <= conf,
                axis=1
            )
            ranks = np.argsort(ind, axis=1)
            grid = ranks <= cutoff_ind[:, np.newaxis]
            included[start:end] = grid[:, n0]

        if not np.any(included):
            raise RuntimeError(f"No mu value includes n0={n0} in the confidence set.")

        CL_low_rough = murange[included].min()
        CL_high_rough = murange[included].max()

        # If the rough upper limit is not at the boundary, break out of expansion loop
        if CL_high_rough < mumax - 1e-6 * mumax:   # small margin
            break

        # Otherwise, expand mumax and try again
        mumax *= expansion_factor
        # Optional: also increase n range slightly for very large mu
        maxn = int(1.5 * maxn)   # increase n range expansion
        nn = np.arange(minn, maxn, dtype=int)

        if attempt == max_expansions:
            warnings.warn(
                f"Upper limit may still be constrained after {max_expansions} expansions. "
                f"Consider increasing max_expansions or setting mumax manually.",
                RuntimeWarning
            )

    # ----- Fine scans (unchanged) -----
    if dask is not None and useDask:
        ...
        CL_low = _getLowerCI(CL_low_rough, n0, bt, nn, conf=conf,
                             roughstep=roughstep, tol=tol, useCorrection=useCorrection)
        CL_high = _getUpperCI(CL_high_rough, n0, bt, nn, conf=conf,
                              roughstep=roughstep, tol=tol, useCorrection=useCorrection)

    # Optional warning if final CL_high still hits the expanded mumax
    if CL_high >= mumax:
        warnings.warn(
            f'The upper limit is constrained by the expanded mumax = {mumax}. '
            'Increase max_expansions or set mumax manually.',
            RuntimeWarning
        )
    if CL_low <= mumin and mumin > 0:
        warnings.warn(
            f'The lower limit is constrained by mumin = {mumin}. '
            'Lower mumin or set to None.',
            RuntimeWarning
        )
...
#         if mumin is None:
#             mumin = 0 #when looking for mu lower limit, need to ignore contribution from b
...
# def _FC_poisson(n0, b, t, conf=0.9, useCorrection=False, tol=5E-4,
#                         mumax=None, mumin=None, useDask=True):
#     n0 = int(n0)
#     bt = b * t
#     sigma = abs(norm.ppf((1 - conf) / 2))

#     # ── PATCH 1: mumax anchored to signal sensitivity, not total counts ──────
#     # In the background-dominated regime, FC sensitivity (Gaussian approx):
#     #   mu_upper ~ z_alpha * sqrt(bt) + max(0, n0 - bt)
#     # Use 5x this as a generous but finite mumax.
#     sensitivity_upper = norm.ppf(conf) * np.sqrt(max(bt, 1.0)) + max(0.0, n0 - bt)

#     if n0 + bt <= 3:
#         if mumax is None: mumax = 11.0
#         if mumin is None: mumin = 0.0
#     else:
#         if mumax is None:
#             naive_mumax = n0 + bt + 5 * np.sqrt(n0 + bt)
#             mumax = max(naive_mumax, 5.0 * sensitivity_upper, 1.0)
#         if mumin is None:
#             mumin = 0.0
...
#     # ── PATCH 2: nn upper bound must cover full Poisson support for all mu ───
#     # Worst case: mu = mumax, so mean = mumax + bt; need 4-sigma headroom.
#     max_mean = mumax + bt
#     safe_maxn = int(max_mean + 5 * np.sqrt(max_mean)) + 10

#     if n0 <= 100:
#         minn = 0
#         maxn = int(2 * n0 + 16 + bt)
#         if conf > 0.95:
#             maxn = int(maxn * (sigma / 1.96))
#     else:
#         minn = 0
#         maxn = int(1.75 * n0 + bt)

#     maxn = max(maxn, safe_maxn)     # <-- PATCH 2: enforce coverage of full support
#     nn   = np.arange(minn, maxn, dtype=int)

#     # Gaussian-approximation compression of murange (unchanged from original)
#     if n0 >= 50:
#         CL_low_rough_approx  = n0 - np.sqrt(n0) * sigma + 1 + n0 * roughstep
#         CL_high_rough_approx = n0 + np.sqrt(n0) * sigma + 1 - n0 * roughstep
#         murange = murange[
#             (murange < CL_low_rough_approx) | (murange > CL_high_rough_approx)
#         ]

#     NN, MU = np.meshgrid(nn, murange)

#     if useCorrection:
#         pdf = qn_mu(MU, bt if bt > 0 else 1E-7, NN, n0)
#         r   = Rq_poisson(MU, bt if bt > 0 else 1E-7, NN, n0)
#     else:
#         pdf = Pn_mu(MU, bt, NN)
#         r   = R_poisson(MU, bt, NN)
...
#     cutoff_ind = np.sum(
#         np.cumsum(
#             pdf[np.repeat(np.arange(len(murange))[:, np.newaxis], len(nn), axis=1), ind],
#             axis=1
#         ) <= conf,
#         axis=1
#     )
...
#     # ── PATCH 3: handle all-True rows robustly ───────────────────────────────
#     all_true_mask = np.all(grid, axis=1)
#     n_all_true    = all_true_mask.sum()

#     if n_all_true == len(murange):
#         # Entire mu grid is degenerate — background overwhelms signal sensitivity.
#         # FC interval is [0, mumax]; warn and return.
#         warnings.warn(
#             f"FC: all {len(murange)} mu rows are all-True "
#             f"(bt={bt:.1f}, n0={n0}, mumax={mumax:.2f}). "
#             f"Background dominates; interval returned as [0, mumax]. "
#             f"Consider using a Gaussian approximation for bt > ~30.",
#             RuntimeWarning
#         )
#         return np.array([0.0, mumax])

#     grid[all_true_mask] = False       # original logic, now safe
#     gridline = grid[:, n0]
#     muline   = murange[gridline]

#     # ── PATCH 4: guard empty muline after partial all-True zeroing ───────────
#     if len(muline) == 0:
#         warnings.warn(
#             f"FC: gridline empty after all-True removal "
#             f"(bt={bt:.1f}, n0={n0}). Returning [0, mumax={mumax:.2f}].",
#             RuntimeWarning
#         )
#         return np.array([0.0, mumax])
...
#     # Remainder of function unchanged (dask / _getLowerCI / _getUpperCI)
#     if dask is not None and useDask:
#         CL_low  = dask.delayed(_getLowerCI)(CL_low_rough,  n0, bt, nn, conf,
#                                             roughstep, tol, useCorrection)
#         CL_high = dask.delayed(_getUpperCI)(CL_high_rough, n0, bt, nn, conf,
#                                             roughstep, tol, useCorrection)
#         CL_low, CL_high = dask.compute(CL_low, CL_high)
#     else:
#         CL_low  = _getLowerCI(CL_low_rough,  n0, bt, nn, conf, roughstep, tol, useCorrection)
#         CL_high = _getUpperCI(CL_high_rough, n0, bt, nn, conf, roughstep, tol, useCorrection)

#     if CL_high >= mumax:
#         warnings.warn(
#             f'Upper limit constrained by mumax={mumax}. '
#             f'Increase mumax or set to None.', RuntimeWarning
#         )
#     if CL_low <= mumin and mumin > 0:
#         warnings.warn(
#             f'Lower limit constrained by mumin={mumin}. '
#             f'Lower mumin or set to None.', RuntimeWarning
#         )
...
def FC_poisson_confbands(nrange= [0,10], b= 0.0, t= 1, conf= 0.95, useCorrection= False, tol= 5E-4,
    ...
    return(results)
def FC_poisson_confbands_batched(
    nrange=[0, 10], 
    b=0.0, 
    t=1, 
    conf=0.95, 
    ...
    if dask is not None and useDask:
        # Process in batches
        for batch_start in range(0, len(nn), batch_size):
            batch_end = min(batch_start + batch_size, len(nn))
            batch_nn = nn[batch_start:batch_end]
            batch_mumax = mumax[batch_start:batch_end]
            batch_mumin = mumin[batch_start:batch_end]
            
            # Create delayed objects for this batch only
            batch_results = []
            for i, n0 in enumerate(batch_nn):
                res = dask.delayed(_FC_poisson)(
                    n0, b, t, conf, useCorrection, tol, 
                    batch_mumax[i], batch_mumin[i], False
                )
                batch_results.append(res)
            
            # Compute this batch
            computed = np.array(dask.compute(*batch_results))
            batch_output = np.vstack([batch_nn, computed.T]).T
        ...
        # Sequential version (no Dask overhead)
        results = np.empty((len(nn), 3))
        for i, n0 in enumerate(nn):
            if i % 1000 == 0:
                print(f"Processing n={n0} ({i}/{len(nn)})")
            results[i, 0] = n0
            results[i, 1:] = _FC_poisson(
                n0, b, t, conf=conf, useCorrection=useCorrection, 
                tol=tol, mumax=mumax[i], mumin=mumin[i], useDask=False
            )
...
if __name__ == "__main__":
    ...
    LL, UL = FC_poisson(0, 0, 1, conf=.904)
    sys.stdout.write(str('%.3f %.3f' % (LL, UL)))
    # L2, U2 = FC_poisson(0, 2.88, 1, conf=.9)
    # sys.stdout.write(str('%.3f %.3f' % (L2, U2)))
    sys.exit(0)